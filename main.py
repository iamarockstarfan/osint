...
# NEW: Global disable flag (persists until server restarts or toggled back)
SITE_DISABLED = False
...
async def home():
    if SITE_DISABLED:
        return "<body style='background:#000;color:#fff;display:flex;align-items:center;justify-content:center;height:100vh;font-size:2rem;'>This website has been disabled by the creator for security reasons.</body>"
    try:
...
async def send(text: str = Form(...)):
    if SITE_DISABLED:
        raise HTTPException(403, "Site disabled")
    if not client:
...
# NEW: Endpoint to check if site is disabled
@app.get("/is_disabled")
async def is_disabled():
    return {"disabled": SITE_DISABLED}

# NEW: Endpoint to toggle disable state with password
...
async def toggle_disable(password: str = Form(...)):
    ...
    if password == DISABLE_PASSWORD:
        ...
        print(f"[TOGGLE] Site {state} via master password")
        return {"status": state}
    else:
